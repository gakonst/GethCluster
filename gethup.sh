...
if [ ! -d "$root/keystore/$dd" ]; then
  ...
  $GETH --datadir $datadir --password /home/dev/dev/web_dev/eth-utils/password.sec  account new &> /dev/null
...
echo "Extra Arguments" $*
 $GETH \
   ...
   --port $port \
   --unlock 0 \
   --password /home/dev/dev/web_dev/eth-utils/password.sec $* \
