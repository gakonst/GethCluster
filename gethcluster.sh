...
if [ ! -f $dir/nodes  ]; then
  ...
  for ((i=0;i<N;++i)); do
	...
	# Check if genesis block has been initialized
...
for ((i=0;i<N;++i)); do
  ...
  echo ./gethup.sh $dir $id $genesis $passwordfile --networkid $network_id $*
  ./gethup.sh $dir $id $genesis $passwordfile --networkid $network_id  &
done
